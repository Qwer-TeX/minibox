...
/* FIXME: Output is broken and doesn't display info in their correct respective columns */
int w(void) {
    struct sysinfo si;
    struct utmp *ut;
    struct tm *tm;
    ...
    while ((ut = getutent())) {
        if (ut->ut_type == USER_PROCESS) {
            ...
            printf("%-8s %-8s %-16s %s  ", ut->ut_user, ut->ut_line, ut->ut_host[0] != '\0' ? ut->ut_host : "-", buf);

            // Calculate idle time
            snprintf(buf, sizeof(buf), "/proc/%d/stat", ut->ut_pid);
            if ((fp = fopen(buf, "r"))) {
                long unsigned int starttime, utime, stime;
                if (fscanf(fp, "%*d %*s %*c %*d %*d %*d %*d %*d %*u %*u %*u %*u %*u %lu %lu %*d %*d %*d %*d %*d %*d %*d %*d %*d %*d %*u %lu", &utime, &stime, &starttime) == 3) {
                    fclose(fp);
                    printf("%-6s ", "IDLE");
                    printf("%-6s ", "JCPU");
                    printf("%-6s ", "PCPU");
